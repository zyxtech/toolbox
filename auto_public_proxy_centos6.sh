...
if [ ! "`rpm -qa|grep privoxy`" ];then
	sed -i 's/^proxy=http:\/\/0.0.0.0:8118\/$/#proxy=http:\/\/0.0.0.0:8118\//' /etc/yum.conf
...
if [ `sed -n '/^#proxy=http:\/\/0.0.0.0:8118\/$/p' /etc/yum.conf ` ];then
	sed -i 's/^#proxy=http:\/\/0.0.0.0:8118\/$/proxy=http:\/\/0.0.0.0:8118\//' /etc/yum.conf
else
	echo "proxy=http://0.0.0.0:8118/" >> /etc/yum.conf
fi
if [ ! "`sed -n "/http_proxy/p" /etc/profile`" ];then
	echo "export http_proxy=\"http://0.0.0.0:8118/\"" >> /etc/profile
	echo "export https_proxy=\"http://0.0.0.0:8118/\"" >> /etc/profile
fi
